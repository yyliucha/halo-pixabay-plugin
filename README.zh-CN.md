...
## 与官方插件的关系与应用市场现状（重要说明）

Halo 官方生态中已有 [plugin-image-stream](https://github.com/halo-sigs/plugin-image-stream)，提供「附件选择器内搜索并转存图片」的能力（支持 Unsplash / Pixabay / Pexels，**内置 API Key 开箱即用**）。本插件与其**核心类别相同**（图片平台 → Halo 附件库），功能存在重叠：

| 场景 | 官方 plugin-image-stream | 本插件 |
|---|---|---|
| 编辑器/附件选择器内搜索并转存 | ✅ 核心能力（内置 Key，零门槛） | ❌ |
| 转存时压缩、选择策略/分组 | ✅ | ❌（批量下载时可选） |
| 定时批量扩充图库 | ❌ | ✅ |
| 关键词 → 存储策略/分组归档 | ❌ | ✅ |
| 发布文章自动配图 | ❌ | ✅ |

因同类别竞争、且对方具备平台生态优势（内置 Key、零门槛、官方维护），**本项目已决定暂缓提交 Halo 应用市场上架**，以**自用 + GitHub Release 分发**为主（见 [Releases](https://github.com/yyliucha/halo-pixabay-plugin/releases)）。本插件的价值定位是**素材自动化**：无人值守地扩充图库、按关键词归档、发布文章自动配图，适合个人博客的内容运营场景。

> 若未来重启上架，候选方向为「文章配图工作流」：以文章为主要消费场景（发布自动配图 + 历史文章批量补图 + 按内容分类扩充图库），与官方「编辑器内手动搜索转存」彻底区隔。详见 [FEATURES_PENDING.md](FEATURES_PENDING.md)。
